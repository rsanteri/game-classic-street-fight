...
type ConsoleModel = 
    { mutable text : string; mutable log : ConsoleItem list }

let console =
    { text = ""; log = [ { itemType = Basic; message = "System started"; time = System.DateTime.Now } ] }
    

let AddItem msg =
    console.log <- List.append console.log [ { itemType = Basic; message = msg; time = System.DateTime.Now } ]

let AddToText char =
    console.text <- console.text + char

let RemoveFromText () =
    if console.text.Length > 0 then
        console.text <- console.text.Remove(console.text.Length - 1, 1)
...
let TakeLast int list =
    list |> List.rev |> List.take int |> List.rev
...
let DrawConsoleLog (spriteBatch: Graphics.SpriteBatch) =
    ...
    for log in ( if List.length console.log > 32 then TakeLast 32 console.log else console.log) do
        spriteBatch.DrawString (ResourceManager.getFont "console", datedText log, Vector2(25.0f, 15.0f * float32 i), Color.White)
        i <- i + 1
    ...
    /// Draw input cursor
    spriteBatch.Draw(ResourceManager.getSprite "box", Rectangle(25 + (console.text.Length * 8), 15 * 32, 3, 20), Color.White)