...
let IsBasePosition(bodypart: EntityBodyPart) = bodypart.position = bodypart.basePosition
...
let HitAction entity entities =
    ...
    if IsBodyPartTooFar hand
    then entity.action <- HitRecovery
    else hand.velocity.x <- hand.velocity.x + (if entity.facing = Right then 10 else -10)
    ...
    | Some npc -> npc.properties.health <- npc.properties.health - 1
    | None -> ignore 0
...
let BodyPartRecovery entity =
    let hand = entity.body.hand
    if hand.position.x <> hand.basePosition.x then HitRecoveryAction entity
...
let HandleEntityAction entity entities =
    ...
    | NoOp -> BodyPartRecovery entity
    | Hit -> HitAction entity entities
...
let UpdateLegMovement entity isLeftLegUp =
    ...
    if leftTooFar
    then lleg.velocity.y <- 0
    else lleg.velocity.y <- lleg.velocity.y + (if isLeftLegUp then -3 else 3)

    if rightTooFar
    then rleg.velocity.y <- 0
    else rleg.velocity.y <- rleg.velocity.y + (if isLeftLegUp then 3 else -3)
...
let UpdateEntityWalking(entity: Entity) =
    ...
    if shouldBeWalking then
        ...
        | NotWalking -> entity.walking <- LeftLegUp
        | LeftLegUp -> UpdateLegMovement entity true
        | RightLegUp -> UpdateLegMovement entity false
    else
        entity.walking <- NotWalking

        /// Return left leg to base position
        if not (IsBasePosition entity.body.lleg) then entity.body.lleg.position <- entity.body.lleg.basePosition

        /// Return right leg to base position
        if not (IsBasePosition entity.body.rleg) then entity.body.rleg.position <- entity.body.rleg.basePosition
